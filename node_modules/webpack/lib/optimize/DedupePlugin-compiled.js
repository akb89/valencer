	...
	compiler.plugin("compilation", function (compilation) {
		...
		compilation.plugin("after-optimize-tree", function (chunks, modules) {
			...
			entryChunks.forEach(function (chunk) {
				...
				(function x(dups, roots, visited, chunk) {
					...
					chunk.modules.forEach(function (module) {
						if (module.duplicates) {
							if (!module.rootDuplicatesChunks) module.rootDuplicatesChunks = module.chunks.slice();
					...
					chunk.chunks.forEach(function (chunk) {
						if (visited.indexOf(chunk) < 0) x(dups, roots, visited.concat(chunk), chunk);
					});
				...
				if (hasDeduplicatedModules) chunk.__DedupePluginHasDeduplicatedModules = true;
			});
		});
		...
		compilation.moduleTemplate.plugin("package", function (moduleSource, module, chunk) {
			...
			if (rootDuplicates.template) {
				...
				var array = [JSON.stringify(rootDuplicates.template.id)].concat(module.getTemplateArguments(rootDuplicates.template.templateModules).map(function (module) {
					if (module.id === null || module.id === undefined) return "(function webpackMissingModule() { throw new Error(" + JSON.stringify("Cannot find module") + "); }())";
		...
		compilation.plugin("chunk-hash", function (chunk, hash) {
			if (chunk.__DedupePluginHasDeduplicatedModules) hash.update("DedupePlugin (deduplication code)");
		});
		...
		compilation.mainTemplate.plugin("modules", function (orginalSource, chunk) {
			...
			source.add(this.indent(["// Check all modules for deduplicated modules", "for(var i in modules) {", this.indent(["if(Object.prototype.hasOwnProperty.call(modules, i)) {", this.indent(["switch(typeof modules[i]) {", "case \"function\": break;", "case \"object\":", this.indent(["// Module can be created from a template", "modules[i] = (function(_m) {", this.indent(["var args = _m.slice(1), fn = modules[_m[0]];", "return function (a,b,c) {", this.indent(["fn.apply(this, [a,b,c].concat(args));"]), "};"]), "}(modules[i]));", "break;"]), "default:", this.indent(["// Module is a copy of another module", "modules[i] = modules[modules[i]];", "break;"]), "}"]), "}"]), "}", "return modules;"]));
			source.add("\n}(");
...
//# sourceMappingURL=DedupePlugin-compiled.js.map