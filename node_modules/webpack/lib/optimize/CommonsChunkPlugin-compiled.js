			...
			commonChunks.forEach(function processCommonChunk(commonChunk, idx) {
				...
				if (async) {
					reallyUsedChunks.forEach(function (chunk) {
						if (chunk.initial || chunk.entry) return;
					...
					usedChunks.forEach(function (chunk) {
						...
						if (chunk.initial) commonChunk.initial = true;
						if (chunk.entry) {
				...
				if (filenameTemplate) commonChunk.filenameTemplate = filenameTemplate;
			}, this);
...
//# sourceMappingURL=CommonsChunkPlugin-compiled.js.map